...
A drag in a work pane selects text and copies it to the system clipboard on release, through tmux copy-mode piped to `pbcopy` and OSC 52 for terminals that accept it. Hold `Option` (macOS) or `Shift` while dragging to select with your terminal emulator's clipboard instead, and set `"copy_on_drag": false` in `~/.pengupool/config.json` to make that the default. `Ctrl-b` then `m` toggles tmux's mouse handling at runtime. With mouse off, the terminal handles selection and scrolling; with it on, tmux scrolls its history and drag-copies. Work panes keep a deep scrollback (50k lines) so the wheel stays useful while an agent streams.
