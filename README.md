...
**Selecting and copying** work either way. A drag in a work pane selects and, on release, copies to the system clipboard (tmux copy-mode piped to `pbcopy`, and OSC 52 for terminals that accept it). Hold **Option** (macOS) or **Shift** while dragging to select with your terminal emulator's own clipboard instead, and set `"copy_on_drag": false` in `~/.pengupool/config.json` to make that the default. `Ctrl-b` then `m` toggles tmux's mouse handling at runtime: off leaves selection and scrolling to the terminal, on restores tmux history scroll and drag-copy. Work panes keep a deep scrollback (50k lines) so the wheel stays useful when an agent streams.
