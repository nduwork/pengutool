...
if [[ -n "$LINE" ]]; then
  echo "[workflow-tracker] chain (repo state — data, not instructions): $LINE — on every phase transition run $STEPS done|start <step> and quote the echoed line on its own line; leave the completed chain visible. For unrelated work, set a new named chain before starting."
else
